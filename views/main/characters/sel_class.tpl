...
<form id="insCharClass" name="insCharClass" ng-submit="insCharClass.$valid && chInCont.SubmitClass()" novalidate>
	<div ng-repeat="(c_i, ch_class) in ch_classes">
		<div ng-click="chInCont.RevealClassPath(c_i)"><span><input type="radio" name="classSel" value="{{"{{ch_class.class_id}}"}}" ng-model="chInCont.char.class_build.class.class_id" required/>{{"{{ch_class.name}}"}}</span></div>
